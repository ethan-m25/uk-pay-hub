...
# === Phase 3 discovery — hub: uk (added 2026-05-27) ===
log "--- Phase 3 discovery: hub_search_jobs ---"
python3 "$HOME/shared-scripts/hub_search_jobs.py" --hub "uk" >> "$LOG_FILE" 2>&1 || true
log "hub_search_jobs done (exit $?)"
log "--- Phase 3 discovery: hub_deep_search ---"
python3 "$HOME/shared-scripts/hub_deep_search.py" --hub "uk" >> "$LOG_FILE" 2>&1 || true
log "hub_deep_search done (exit $?)"
log "--- Phase 3 discovery: hub_search_kpmg ---"
python3 "$HOME/shared-scripts/hub_search_kpmg.py" --hub "uk" >> "$LOG_FILE" 2>&1 || true
log "hub_search_kpmg done (exit $?)"
log "--- Phase 3 discovery: hub_search_sap ---"
python3 "$HOME/shared-scripts/hub_search_sap.py" --hub "uk" >> "$LOG_FILE" 2>&1 || true
log "hub_search_sap done (exit $?)"
log "--- Phase 3 discovery: hub_search_scout ---"
python3 "$HOME/shared-scripts/hub_search_scout.py" --hub "uk" >> "$LOG_FILE" 2>&1 || true
log "hub_search_scout done (exit $?)"

# === Phase 5 classify — hub: uk (added 2026-05-28) ===
log "--- Phase 5 classify_step ---"
python3 "$HOME/shared-scripts/region_classifier/classify_step.py" \
    --hub "uk" --no-llm >> "$LOG_FILE" 2>&1 || true
log "classify_step done (exit $?)"
# === Phase 5.5 publish gate — hub: uk (added 2026-05-28) ===
# Filters today's raw to only classifier-claimed rows before update-jobs reads it.
# Safety: gate refuses (exit 1) if drop_pct > 10.0%; raw file untouched on refusal.
GATE_RAW="$HOME/.openclaw/shared/uk-jobs-raw-$(date +%Y-%m-%d).txt"
if [[ -f "$GATE_RAW" ]]; then
...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only --hub "uk" >> "$LOG_FILE" 2>&1
...
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only \
  --hub uk --pipeline-exit-code "$PIPELINE_RC" || true
exit "$PIPELINE_RC"
