...
if git diff --cached --quiet; then
  notify_discord "ℹ️ UK Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: UK daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
git push origin main 2>/dev/null || true

export PATH="/Users/clawii/.npm-global/bin:$PATH"
wrangler pages deploy . --project-name uk-payhub --branch main 2>&1 | tail -3 || true
...
notify_discord "✅ UK Pay Hub updated [$TODAY]
