#!/usr/bin/env python3
"""
uk-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — United Kingdom edition.
...
Run: python3 ~/uk-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/uk-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/uk-pay-hub/scripts/.greenhouse.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = [('monzo', None), ('skyscanner', None), ('checkout', 'Checkout.com'), ('farfetch', None), ('deliveroo', None), ('wise', None), ('starlingbank', 'Starling Bank'), ('darktrace', None), ('funding-circle', 'Funding Circle'), ('beamery', None), ('tractable', None), ('revolut', None), ('gocardless', 'GoCardless')]

SALARY_PATTERNS = [
    r'\£\s*([\d,]+)\s*[-–—]\s*\£\s*([\d,]+)',
    r'GBP\s*([\d,]+)\s*[-–—]\s*GBP\s*([\d,]+)',
    r'([\d,]+)\s*[-–—]\s*([\d,]+)\s*(?:GBP|gbp)',
    ...
    for pat in SALARY_PATTERNS:
        ...
        if m:
            try:
                ...
                if 20000 <= val_min < val_max <= 1000000:
...
def main():
    ...
    log("=== UK Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new UK jobs written to {OUTPUT_FILE} ===")
