#!/usr/bin/env python3
"""
uk-pay-hub/scripts/search-lever.py
Lever job board scraper — United Kingdom edition.
...
Run: python3 ~/uk-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/uk-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/uk-pay-hub/scripts/.lever.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = ['matillion', 'zen-educate', 'monzo', 'gocardless', 'thought-machine', 'marshmallow', 'oaknorth', 'funding-circle', 'graze', 'tractable', 'beamery', 'marshmallow-insurance', 'nested', 'onfido']
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))

DISCOVERY_QUERIES = ['site:jobs.lever.co "London" OR "United Kingdom" "£" salary 2025 2026', 'site:jobs.lever.co "London" OR "Manchester" engineer analyst GBP salary', 'site:jobs.lever.co "UK" OR "England" pay range "£" engineer']

SALARY_RE = [
    re.compile(r'\£\s*([\d,]+)(?:\.\d+)?\s*[-–—]+\s*\£\s*([\d,]+)', re.IGNORECASE),
    re.compile(r'\$([\d]+(?:\.\d+)?)[kK]\s*[-–—]\s*\$([\d]+(?:\.\d+)?)[kK]', re.IGNORECASE),
    re.compile(r'(?:pay|salary|compensation|base|wage|range)[^£\n]{0,50}£?([\d,]{4,})\s*[-–—]+\s*£?([\d,]{4,})', re.IGNORECASE),
    re.compile(r'GBP\s*([\d,]+)\s*[-–—]\s*GBP\s*([\d,]+)', re.IGNORECASE),
]
...
def extract_salary_from_range(sal_range):
    ...
    try:
        ...
        if 20000 <= vmin <= 1000000 and vmin < vmax:
            return vmin, vmax
...
def main():
    ...
    log(f"\n=== Lever scraper complete: {total_found} new UK jobs (api_failures={api_failures}) ===")
    return 0
